...
TNo *cria(int d) {
    ...
    novo->m = 0;
    novo->pai = NULL;
    novo->s = (int *) malloc(sizeof(int *) * (d * 2 - 1));
    novo->p = (TNo **) malloc(sizeof(TNo *) * (d * 2));
    for (int i = 0; i < (d * 2); i++) {
        novo->p[i] = NULL;
    }
    return novo;
...
void insere(TNo *no, int d, int pos, int chave, TNo *pt) {
    ...
    if (no->m < 2 * d - 1) { // Tem espaço no nó, então pode inserir diretamente
...
TNo* BuscaArvoreB(TNo *raiz,int alvo){

    TNo *noh = busca(raiz,alvo);

    int pos = posicao(alvo,noh);
